...
class JobStore:
    ...
    def submit(
        self, path: Path, options: dict[str, Any], cleanup_path: Path | None = None
    ) -> Job:
        job = Job(
    ...
    def _run(self, job: Job, path: Path, options: dict[str, Any]) -> None:
        ...
        try:
            ...
            def work() -> None:
                try:
                    outcome["result"] = parse_file(path, options)
            ...
            worker = threading.Thread(target=work, name=f"convert-{job.id}", daemon=True)
            worker.start()
            while not done.wait(timeout=0.25):
                ...
                if time.time() >= deadline:
                    self._fail(
                        job,
                        "timeout",
                        f"parse exceeded the {TIMEOUT_SECS}s limit — the first parse on "
                        "this node also downloads Docling's layout and table models, so "
                        "a cold run can be much slower than a warm one; raise "
                        "RYU_DOCLING_TIMEOUT_SECS or retry once the models are cached",
                    )
                    return
