...
1. `resolve_input` — the parse request names a file by *path* (Core hands over a
   content-addressed blob under `<RYU_DIR>/blobs/…`, never an upload), so the path
...
def allowed_roots() -> list[Path]:
    ...
    `RYU_DOCLING_ROOTS` is an `os.pathsep`-separated list Core sets from the
    manifest (`${RYU_DIR}` is the only token the manifest may interpolate). The
    fallback order matters: `RYU_DIR` is the reserved env Core injects from its
    own profile-aware `paths::ryu_dir()`, so it is right under `bun dev`
    (`~/.ryu-dev`) as well as release. `~/.ryu` is last and is only reached on a
    bare standalone run — bottoming out there first would reject every blob parse
    on a dev profile and nowhere else, which is exactly the kind of bug a
    release-profile smoke test cannot see. An empty allow-list must never mean
    "everything".
    """
    raw = (os.environ.get("RYU_DOCLING_ROOTS") or "").strip()
    if not raw:
        raw = (os.environ.get("RYU_DIR") or "").strip()
    if not raw:
        raw = str(Path.home() / ".ryu")
    roots: list[Path] = []
...
def resolve_input(raw_path: str) -> Path:
    ...
    if not any(_is_within(resolved, root) for root in roots):
        ...
        raise InputError(
            ...
            "set RYU_DOCLING_ROOTS to widen them"
        )
    ...
    if size > MAX_INPUT_BYTES:
        raise InputError(
            ...
            "(raise RYU_DOCLING_MAX_INPUT_BYTES to allow it)"
        )
    return resolved
